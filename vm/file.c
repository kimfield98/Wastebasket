...
do_mmap (void *addr, size_t length, int writable,
	...
	while (read_bytes > 0 || zero_bytes > 0)
	{
		...
		struct lazy_aux *lazy_load_arg = (struct lazy_aux*)malloc(sizeof(struct lazy_aux));
		...
		if (!vm_alloc_page_with_initializer(VM_FILE, addr,writable, lazy_do_mmap, lazy_load_arg))
			return NULL;
