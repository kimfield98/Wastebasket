...
function Home() {
  ...
  const [mode, setMode] = useState('TIL');
  const [id, setId] = useState(null);
  const [nextId, setNextId] = useState(4);

  const [topics,setTopics] = useState([
    {id:1, title:'11/01', body:'리액트 환경설정'},
    {id:2, title:'11/02', body:'리액트 게시판 만들기'}
  ]);
  ...
  if(mode==="TIL"){
    content = <Article title="TIL" body="오늘 공부한 내용을 기록해보세요!"></Article>
    contextControl = <>
  ...
  return (
    <>
      <Header title="✍️게시판" onChangeMode={() => {
        setMode('TIL');
      }}></Header>
